...
void SearchDialog::on_btnSearch_clicked()
{
    ...
    // Query the database
    bool resourceExists = queryDatabase(resourceName);

    // Send result to Arduino (or log for simulation)
    sendToArduino(resourceExists);

    // Update UI
    ui->lblStatus->setText(resourceExists ? "Resource '" + resourceName + "' exists!" :
                               "Resource '" + resourceName + "' does not exist.");
    QMessageBox::information(this, "Search Result",
                             resourceExists ? "Resource '" + resourceName + "' exists!" :
                                 "Resource '" + resourceName + "' does not exist.");
}

bool SearchDialog::queryDatabase(const QString &resourceName)
{
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM RESOURCES WHERE UPPER(NAME) = UPPER(:name)");
    query.bindValue(":name", resourceName);
    if (query.exec() && query.next()) {
        int count = query.value(0).toInt();
        qDebug() << "Database query result: Resource" << resourceName << (count > 0 ? "exists" : "does not exist");
        return count > 0;
    } else {
...
{
    if (serialPort->isOpen()) {
        serialPort->write(resourceExists ? "1" : "2");
        serialPort->flush();
        qDebug() << "Sent to Arduino:" << (resourceExists ? "1" : "2");
    } else {
        // Log command for simulation
        ...
        QMessageBox::information(this, "Simulation", "Please send command '" + command + "' to Wokwi Serial Monitor.");
    }
}
*/
...
void SearchDialog::on_btnSearch_clicked()
{
    ...
    // Query the database and get resource data
    int resourceId = -1;
    Resource resource;
    ...
    if (resourceExists) {
        ...
        if (currentQuantity <= 0) {
            ...
            sendToArduino(false); // Treat as not found for Arduino
            return;
        }
...
bool SearchDialog::updateResourceQuantity(int resourceId, int currentQuantity, const QString &employeeName)
{
    ...
    // Retrieve other resource data to maintain it during update
    QSqlQuery query;
